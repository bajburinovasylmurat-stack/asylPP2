...
function deleteEvent(id) {
  ...
  getDb().get('seats').remove({ eventId: id }).write();
  return true;
}
...
// Admin: дауыс қосу / алу
function adminAddVote(eventId, regionChoice) {
  const vote = { id: uuid(), eventId, regionChoice, token: 'admin_' + uuid(), createdAt: new Date().toISOString() };
  getDb().get('votes').push(vote).write();
  return vote;
}
function adminRemoveVote(eventId, regionChoice) {
  const votes = getDb().get('votes').filter({ eventId, regionChoice }).value();
  if (!votes.length) return false;
  const last = votes[votes.length - 1];
  getDb().get('votes').remove({ id: last.id }).write();
  return true;
}

// ─── Seats ───────────────────────────────────────────────────
function getSeatsByEvent(eventId) {
  return getDb().get('seats').filter({ eventId }).value();
}
function getSeatById(id) {
  return getDb().get('seats').find({ id }).value();
}
function createSeat(data) {
  ...
  getDb().get('seats').push(seat).write();
  return seat;
}
function updateSeat(id, data) {
  getDb().get('seats').find({ id }).assign(data).write();
  return getSeatById(id);
}
function deleteSeat(id) {
  if (!getSeatById(id)) return false;
  getDb().get('seats').remove({ id }).write();
  return true;
}
function deleteSeatsByEvent(eventId) {
  getDb().get('seats').remove({ eventId }).write();
}
function toggleSeatStatus(id) {
  const seat = getSeatById(id);
  if (!seat) return null;
  const newStatus = seat.status === 'taken' ? 'free' : 'taken';
  getDb().get('seats').find({ id }).assign({ status: newStatus }).write();
  return getSeatById(id);
}
function bulkSetSeatsStatus(eventId, status) {
  getDb().get('seats').filter({ eventId }).each(s => { s.status = status; }).write();
}
...
module.exports = {
  ...
  getVotesByEvent, getAllVotesSummary, tokenExists, createVote, deleteVotesByEvent, adminAddVote, adminRemoveVote,
  getSeatsByEvent, getSeatById, createSeat, updateSeat, deleteSeat, deleteSeatsByEvent, toggleSeatStatus, bulkSetSeatsStatus,
