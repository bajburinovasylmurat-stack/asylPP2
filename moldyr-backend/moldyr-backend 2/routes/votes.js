...
// Admin: дауыс қосу (+1 / -1)
router.post('/:eventId/adjust', requireAuth, (req, res) => {
  const { regionChoice, delta } = req.body;
  if (![1,2].includes(regionChoice) || ![1,-1].includes(Number(delta)))
    return res.status(400).json({ error: 'regionChoice (1|2) және delta (1|-1) міндетті' });

  const ev = db.getEventById(req.params.eventId);
  if (!ev) return res.status(404).json({ error: 'Табылмады' });

  if (Number(delta) === 1) {
    db.adminAddVote(ev.id, regionChoice);
  } else {
    const ok = db.adminRemoveVote(ev.id, regionChoice);
    if (!ok) return res.status(400).json({ error: 'Алып тастайтын дауыс жоқ' });
  }
